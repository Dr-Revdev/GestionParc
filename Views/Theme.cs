...
public static class Theme
{
    ...
    /// <summary>
    /// Applique le style à un ListView avec bordure noire épaisse
    /// </summary>
    public static ListView StyleListView(ListView listView)
    {
        listView.Font = Fonts.Body;
        listView.BackColor = Colors.Surface;
        listView.ForeColor = Colors.TextPrimary;
        
        // Utiliser une bordure noire visible
        listView.BorderStyle = BorderStyle.None;
        
        // Ajouter le handler Paint pour dessiner la bordure noire
        listView.Paint += (sender, e) => {
            using (Pen pen = new Pen(Color.Black, 2))
            {
                e.Graphics.DrawRectangle(pen, 0, 0, listView.Width - 1, listView.Height - 1);
            }
        };
        
        // Redessiner quand la taille change
        listView.Resize += (s, e) => listView.Invalidate();
        
        return listView;
    }
    
    /// <summary>
    /// Applique le style à un ListBox avec bordure noire épaisse
    /// </summary>
    public static ListBox StyleListBox(ListBox listBox)
    {
        ...
        // Redessiner quand la taille change
        listBox.Resize += (s, e) => listBox.Invalidate();
